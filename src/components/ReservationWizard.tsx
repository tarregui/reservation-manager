...
function ReservationWizard() {
  ...
  // Animación CSS basada en dirección
  ...
  return (
    <div className="w-full">
      {/* Progress Bar - FUERA del card para que siempre sea visible */}
      <div className="mb-6">
        <div className="grid grid-cols-4 gap-0">
          {[1, 2, 3, 4].map((s, idx) => (
            <div key={s} className="flex flex-col items-center relative">
              {idx > 0 && (
                <div
                  className={`absolute top-4 left-[-50%] w-full h-1 transition-all duration-300 ${
                    step >= s ? 'bg-orange-500' : 'bg-gray-300'
                  }`}
                />
              )}
              <div
                className={`z-10 w-8 h-8 rounded-full flex items-center justify-center font-semibold text-sm transition-all duration-300 ${
      ...
      <div className="bg-white rounded-2xl shadow-xl p-6 overflow-hidden" style={{ minHeight: '450px' }}>
        <div key={step} className={getAnimationClass()}>
          ...
          {step === 4 && (
            <div className="flex flex-col" style={{ minHeight: '400px' }}>
              ...
              <div className="bg-orange-50 rounded-xl p-4 mb-4 space-y-3 flex-1">
